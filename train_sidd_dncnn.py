...
parser.add_argument("--r2r_path", type=str, default='./', help="path of R2R training data")
...
def main():
    # Load dataset
    current_time = time.strtime('%Y%m%d-%H:%M:%s',time.localtime())
    ...
    loader_train = DataLoader(dataset=dataset_train, num_workers=4, batch_size=32, shuffle=False)
    ...
    for epoch in range(opt.epochs):
        ...
        for i, (r2r_intput,r2r_output) in enumerate(loader_train, 0):
            ...
            print("%s [epoch %d][%d/%d] loss: %.4f" %
                (opt.training,epoch+1, i+1, len(loader_train), loss.item()),now.strftime("%H:%M:%S"))
        ...
        print('epoch: %d validation: %f %f' %(epoch,psnr_val/40/32))               
